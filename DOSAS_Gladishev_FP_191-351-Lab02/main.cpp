...
int main(int argc, char *argv[])
{
    ...
    // БЛОК ЗАЩИТЫ ОТ ОТЛАДКИ МЕТОДОМ САМООТЛАДКИ
        ...
        //qDebug() << "Protector Started: = " << ProtectorStarted;
    ...
    // вызывается WinAPI-функция GetModuleHandle
        QWORD moduleBase = (QWORD)GetModuleHandle(NULL); // начю адрес, с которого приложение разм. в вирт. памяти
        QWORD text_segment_start = moduleBase + 0x1000; // сегмент .text располагается с отступом 0x1000 по дефолту
        qDebug() << "text_segment_start = " << Qt::hex << text_segment_start;

    // Определяется длина сегмента из заголовка
        ...
        const QByteArray hash0_base64 = QByteArray("jpnBoCccfkJmqx/BoEnrOiMNi/BlBxCmYFFmOwkA4Fw="); //задаем константу, в которой будет храниться эталонный хэш, лежит в сегменте с константами .data

        //для проверки выводим все данные в отладку
        qDebug() << "size_of_text = " << size_of_text;
        qDebug() << "text_segment_contents = " << Qt::hex << text_segment_contents.first(100); // первые 100 символов сегмента .text
